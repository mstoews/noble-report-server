DB_URL=postgresql://root:secret@localhost:32342/postgres?sslmode=disable

network:
	docker network create bank-network

postgres:
	docker run --name prd-backup --network bank-network -p 32342:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=secret -d postgres:14-alpine
